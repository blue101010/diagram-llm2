...
A project for generating Mermaid diagrams using large language models.

## Problem Identification

Generative AI models, while excelling in generating high-quality text and code outputs, struggle with the automated creation of academic and flow diagrams. These diagrams, often essential for conveying complex architectures and processes, are predominantly generated manually, resulting in time-consuming and error-prone workflows. The lack of a high-quality, publicly available dataset that pairs textual descriptions with corresponding diagram representations—specifically in the mermaid diagram format—stymies progress in applications such as automated technical documentation, educational visualization, and rapid prototyping for system designers.

The issue manifests in various critical areas:

- **Academic Documentation:** Visual diagrams help students and researchers quickly grasp complex concepts but are not readily generated by existing AI tools.
- **Technical Communication:** Engineers and system designers require precise and automated diagram generation to validate and communicate intricate system processes.
- **Rapid Prototyping:** The absence of a tool to quickly translate system descriptions into visual workflows inhibits the iterative design and testing cycles in industry settings.

## Novel Aspect

- There is currently almost no reliable way to may LLMs generate proper academic and architectural diagrams
- ChatGPT and other LLMs are good at code generation, image generation (when asked for example to create an image of a human) and text generation but cannot generate diagrams.
- There is no high quality Text to Diagram (Mermaid) dataset available
- In our research we found the existing datasets to be lagging and they were of poor quality (mostly containing simple diagrams and many errors)

## Methodology

1. **Synthetic Dataset Generation (Using Gemini 2.5 Pro):**

   - **Prepare Documentation:** Gather the official Mermaid documentation specific to the desired diagram type (e.g., flowchart, sequence).
   - **Generate Questions:** Input the documentation into Gemini 2.5 Pro to generate a set of diverse textual questions (prompts) that require the specific Mermaid diagram type as an answer.
...
Initially we were trying to generate dataset with valid question answer pairs like "Generate a diagram for TCP/IP stack?". But we found this introduced a lot of potential for hallucinations while generating the dataset.

So, hence we moved to questions which describe everything deterministically so that there is no scope of hallucinations. We also found this works better to teach model the actual mermaid syntax.
